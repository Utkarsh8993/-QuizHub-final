...
const removeTheUser = asyncHandler(async (req,res)=>{
    if(!req?.body?.id) return res.status(404).json({message : 'The id must be provided'});
    if(!req?.body?.quizId) return res.status(404).json({message : 'The id must be provided'});
    if(!req?.user?._id) return res.status(403).json({ message : 'You must be logged in'});
    try {
        ...
        for (let index = 0; index < userArray.length; index++) {
            ...
            if(element === user._id.toString() ){
                if(user.isLeader) {
                    const quizRoom = QuizRoom.findById(quizId);
                    for (let k = 0; k < quizRoom.Groups.length; k++) {
                        const element = quizRoom.Groups[k].toString();
                        if(element === req.body.id){
                            quizRoom.Groups.splice(k,1);
                            await quizRoom.save();
                        }
                    }
