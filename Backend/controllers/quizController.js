...
const endQuiz = asyncHandler(async(req,res) =>{
    if(!req?.body?.quizId) return res.status(400).json({ message : 'The quiz id is required in the url'});
    try {
        ...
        for (let index = 0; index < GroupsArray.length; index++) {
            ...
            console.log(group)
            const userArray = group.UserArray.map((user) =>{
            ...
            for (let j = 0; j < userArray.length; j++) {
                ...
                if(!result) return res.json({message : 'Unable to Update User'})
            }
            const result = await Groups.findByIdAndDelete(element)
            if(!result) return res.json({message : 'Unable to delete group'})
        }
        const result = await QuizRoom.findByIdAndDelete(req.body.quizId)
        if(!result) return res.status(400).json({message : 'Unable to delete quiz'})
